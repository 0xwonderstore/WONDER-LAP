...
const FilterComponent: React.FC<FilterComponentProps> = ({
  ...
  const handleCustomDateChange = (field: 'startDate' | 'endDate', value: string) => {
    ...
    onDateRangeChange('custom', newStartDate, newEndDate);
  };
  
  const dateFields = [
      { id: 'start-date', label: t.from, value: filters.startDate, onChange: (e: React.ChangeEvent<HTMLInputElement>) => handleCustomDateChange('startDate', e.target.value) },
      { id: 'end-date', label: t.to, value: filters.endDate, onChange: (e: React.ChangeEvent<HTMLInputElement>) => handleCustomDateChange('endDate', e.target.value) }
  ];
  ...
  return (
    <div className="bg-light-surface dark:bg-dark-surface p-4 rounded-2xl mb-6 shadow-md">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-7 gap-4">
        ...
        <div className="flex-grow min-w-[140px]">
          ...
          <div className="relative">
            <select id='store-select' value={filters.store} onChange={(e) => onFilterChange('store', e.target.value)} className="w-full appearance-none cursor-pointer p-2.5 border border-light-border dark:border-dark-border rounded-xl bg-light-background dark:bg-dark-background focus:ring-2 focus:ring-brand-primary text-right ltr:text-left ltr:pr-10 rtl:pl-10">
        ...
        {/* Language Select */}
        <div className="flex-grow min-w-[140px]">
          <label htmlFor='language-select' className="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-1 px-2">{t.selectLanguage}</label>
          <div className="relative">
            <select id='language-select' value={filters.language} onChange={(e) => onFilterChange('language', e.target.value)} className="w-full appearance-none cursor-pointer p-2.5 border border-light-border dark:border-dark-border rounded-xl bg-light-background dark:bg-dark-background focus:ring-2 focus:ring-brand-primary text-right ltr:text-left ltr:pr-10 rtl:pl-10">
        ...
        {/* Date Range Select */}
        <div className="flex-grow min-w-[140px]">
          <label htmlFor='date-range-select' className="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-1 px-2">{t.creationDate}</label>
          <div className="relative">
            <select id='date-range-select' value={filters.dateRange} onChange={(e) => onDateRangeChange(e.target.value)} className="w-full appearance-none cursor-pointer p-2.5 border border-light-border dark:border-dark-border rounded-xl bg-light-background dark:bg-dark-background focus:ring-2 focus:ring-brand-primary text-right ltr:text-left ltr:pr-10 rtl:pl-10">
        ...
        {/* Per Page Select */}
        <div className="flex-grow min-w-[140px] lg:col-start-6 xl:col-start-auto">
          ...
          <div className="relative">
            <select id='per-page-select' value={productsPerPage} onChange={(e) => onProductsPerPageChange(Number(e.target.value))} className="w-full appearance-none cursor-pointer p-2.5 border border-light-border dark:border-dark-border rounded-xl bg-light-background dark:bg-dark-background focus:ring-2 focus:ring-brand-primary text-right ltr:text-left ltr:pr-10 rtl:pl-10">
      ...
      {filters.dateRange === 'custom' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-7 gap-4 mt-4">
            <div className="lg:col-start-4 xl:col-start-5 grid grid-cols-2 gap-4 col-span-1 sm:col-span-2 md:col-span-2 lg:col-span-2">
              {dateFields.map(field => (
                <div key={field.id} className="flex-grow min-w-[120px]">
                  <label htmlFor={field.id} className="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-1 px-2">{field.label}</label>
                  <input type="date" id={field.id} value={field.value} onChange={field.onChange} className="w-full p-2.5 border border-light-border dark:border-dark-border rounded-xl bg-light-background dark:bg-dark-background focus:ring-2 focus:ring-brand-primary"/>
                </div>
              ))}
            </div>
        </div>
