...
const VendorAnalytics: React.FC<VendorAnalyticsProps> = ({ products, onBack }) => {
  ...
  const filteredAndSortedStats = useMemo(() => {
    ...
    if (minProducts > 0) {
        stats = stats.filter(v => v.productCount >= minProducts);
    }
  ...
  const requestSort = (key: keyof VendorStat) => {
  ...
  const SortArrow = ({ for_key }: {for_key: keyof VendorStat}) => {
    ...
    <div className="p-4 sm:p-8 bg-light-background dark:bg-dark-background min-h-screen animate-fade-in">
      ...
      <div className="bg-light-surface dark:bg-dark-surface p-6 rounded-2xl shadow-lg">
        ...
        <div className="flex flex-wrap gap-4 mb-6">
            <div>
                <label htmlFor="minProducts" className="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary">
                    عدد المنتجات (على الأقل)
                </label>
                <input
                    id="minProducts"
                    type="number"
                    ...
                    className="mt-1 block p-2 rounded-lg border-light-border dark:border-dark-border bg-light-background dark:bg-dark-background"
                    placeholder="e.g., 5"
                />
            </div>
             <div>
                <label htmlFor="activityDate" className="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary">
                    آخر نشاط للمتجر
                </label>
                <select
                    id="activityDate"
                ...
                >
                    ...
                    <option value="week">في الأسبوع الأخير</option>
                    <option value="month">في الشهر الأخير</option>
                </select>
            </div>
        </div>
            ...
            <thead className="text-xs text-light-text-secondary dark:text-dark-text-secondary uppercase bg-light-background dark:bg-dark-background">
              <tr>
                  {([
                      ...
                      {key: 'productCount', label: 'عدد المنتجات'},
                      {key: 'oldestProductDate', label: 'أول منتج'},
                      {key: 'newestProductDate', label: 'آخر منتج'},
                  ] as {key: keyof VendorStat, label: string}[]).map(({key, label}) => (
                      <th key={key} scope="col" className="px-6 py-3 cursor-pointer" onClick={() => requestSort(key)}>
                          <div className="flex items-center gap-1">{label} <SortArrow for_key={key}/></div>
                      </th>
                  ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-light-border dark:divide-dark-border">
              {filteredAndSortedStats.map(v => (
                <tr key={v.name} className="hover:bg-light-background dark:hover:bg-dark-background">
                  <td className="px-6 py-4 font-medium text-light-text-primary dark:text-dark-text-primary whitespace-nowrap">{v.name}</td>
